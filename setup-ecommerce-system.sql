...
-- Data: 09 de Janeiro de 2026 - CORRIGIDO
-- ============================================
...
-- RLS Policies - Remover políticas existentes se existirem
DROP POLICY IF EXISTS "Public can view active coupons" ON public.coupons;
DROP POLICY IF EXISTS "Authenticated users can use coupons" ON public.coupons;
DROP POLICY IF EXISTS "Admin can manage coupons" ON public.coupons;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON public.coupons;
...
-- Policy para cupons - Políticas separadas para cada operação
CREATE POLICY "Coupons select policy" ON public.coupons
    FOR SELECT USING (true);

CREATE POLICY "Coupons insert policy" ON public.coupons
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Coupons update policy" ON public.coupons
    FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Coupons delete policy" ON public.coupons
    FOR DELETE USING (auth.uid() IS NOT NULL);

-- Policy para pedidos (usuários podem ver seus próprios pedidos)
...
SELECT 'Sistema de e-commerce criado com sucesso!' as status;